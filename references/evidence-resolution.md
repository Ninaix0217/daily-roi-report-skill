...
Union and consolidation guards read an effective, computed metadata view without rewriting resolver evidence. Effective store scope combines `contexts[].store`, `contexts[].template_store`, and a nonempty `reconciliation.store` only when reconciliation status is `PASS` and no hard store contradiction exists. Conflicting context/reconciliation stores block the relation. Effective semantic roots prefer an explicit root, otherwise use root fields from whitelisted semantic-resolver evidence, and fall back to alias-family normalization only when neither exists; explicit source bridges remain a separate accepted bridge. Conflicting semantic-root provenance blocks the relation, and an unrelated nested `source_root` is ignored. This projection exposes evidence already produced by the production path and does not relax the store, root, alternative-target, or identity-conflict guards.
